...
class DARTDetectorModel:
    ...
    def __init__(
        ...
        # if not Path(checkpoint).is_file():
            # logger.warning(f"Checkpoint file not found locally: {checkpoint}. Attempting download.")
        ...
        self.use_fp16 = (device == "cuda") if use_fp16 is None else use_fp16
    ...
    def warmup(self) -> None:
        ...
        This should not change predictions, only latency.
        """
        if self._warmed_up:
            return
    ...
    def _postprocess(
        ...
        return boxes_xywh.astype(np.float32), scores.astype(np.float32), labels
    ...
    def inference(self, images, shelf_writer=None, profile: bool = False):
        ...
        with torch.inference_mode():
            for item in tqdm(images):
                ...
                boxes_xywh, scores, labels = self._postprocess(boxes_xyxy, scores, labels)
